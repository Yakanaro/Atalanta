            ...
            <div class="mb-6 md:hidden">
                ...
                <div id="mobile-filter-panel" class="hidden mt-2 bg-white dark:bg-gray-800 rounded-lg shadow p-4">
                    <form action="{{ route('dashboard') }}" method="GET" class="space-y-4">
                        ...
                        <div>
                            <label for="mobile_filter_row" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Ряд</label>
                            <input type="number" id="mobile_filter_row" name="filter_row" value="{{ request('filter_row') }}" min="1"
                                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
            ...
            <div class="mb-6 hidden md:block">
                ...
                <div id="desktop-filter-panel" class="bg-white dark:bg-gray-800 rounded-lg shadow p-4 hidden">
                    <form action="{{ route('dashboard') }}" method="GET" class="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-6 gap-4">
                        ...
                        <div>
                            <label for="filter_row" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Ряд</label>
                            <input type="number" id="filter_row" name="filter_row" value="{{ request('filter_row') }}" min="1"
                                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                                placeholder="Номер ряда">
                        </div>
                            ...
                            <td class="px-6 py-4 text-right">
                                <div class="flex justify-end space-x-1 -mr-2">
                                    <a href="{{ route('pallet.show', $pallet->id) }}"
                                        class="text-blue-600 dark:text-blue-500 hover:bg-blue-100 dark:hover:bg-blue-900 p-1 rounded-full transition-colors"
                                        title="Просмотр">
                                    ...
                                    <a href="{{ route('pallet.edit', $pallet->id) }}"
                                        class="text-green-600 dark:text-green-500 hover:bg-green-100 dark:hover:bg-green-900 p-1 rounded-full transition-colors"
                                        title="Редактировать">
                                        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                    ...
                                    <a href="{{ route('stockPosition.create', ['pallet_id' => $pallet->id]) }}"
                                        class="text-purple-600 dark:text-purple-500 hover:bg-purple-100 dark:hover:bg-purple-900 p-1 rounded-full transition-colors"
                                        title="Добавить позицию">
                                        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4в16m8-8H4" />
                                        </svg>
                                    </a>
                                    <button type="button"
                                        data-modal-target="deleteModal-{{ $pallet->id }}"
                                        data-modal-toggle="deleteModal-{{ $pallet->id }}"
                                        class="text-red-600 dark:text-red-500 hover:bg-red-100 dark:hover:bg-red-900 p-0.5 rounded-full transition-colors -ml-1"
                                        title="Удалить">
                                        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                ...
                <div class="bg-white dark:bg-gray-800 rounded-lg shadow overflow-hidden">
                    <div class="p-4 border-b border-gray-200 dark:border-gray-700 flex justify-between items-center">
                        <div>
                            ...
                            <p class="text-xs text-gray-500 dark:text-gray-400">Ряд: <span class="font-medium text-gray-900 dark:text-white">{{ $pallet->row ?: '—' }}</span></p>
                            @php
                            $statusData = $pallet->getStatusWithClass();
                            @endphp
                            <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium {{ $statusData['class'] }}">
